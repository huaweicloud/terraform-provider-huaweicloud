# Kill DCS Redis instance all client sessions

This example provides best practice code for using Terraform to kill all client sessions of a DCS Redis instance in
...
* `kill_all_nodes` - Whether to kill sessions of all nodes, valid values: true, false (default: "false")
...
* When `kill_all_nodes` is "false", `node_id` is required and will be auto-obtained from the instance shards
* When `kill_all_nodes` is "true", `node_id` should not be set
* All parameters are non-updatable, changing any of them will trigger resource replacement
* Deleting this resource only removes it from Terraform state, the kill operation is not reversed
