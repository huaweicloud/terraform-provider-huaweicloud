# DCS Redis Web CLI logout

This example provides best practice code for using Terraform to log out of the Web CLI of a DCS Redis instance.
...
* This resource must be used together with `huaweicloud_dcs_login_web_cli` to form a complete login-logout workflow
* All parameters are non-updatable, changing any of them will trigger resource replacement
* Deleting this resource only removes it from Terraform state, the logout action is not reversed
