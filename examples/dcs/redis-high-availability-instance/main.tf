...
resource "huaweicloud_dcs_instance" "test" {
  ...
  engine_version        = var.instance_engine_version
  capacity              = var.instance_capacity
  flavor                = var.instance_flavor_id == "" ? try(data.huaweicloud_dcs_flavors.test[0].flavors[0].name, null) : var.instance_flavor_id
  availability_zones    = length(var.availability_zones) == 0 ? try(slice(data.huaweicloud_availability_zones.test[0].names, 0, 2), null) : var.availability_zones
  vpc_id                = huaweicloud_vpc.test.id
