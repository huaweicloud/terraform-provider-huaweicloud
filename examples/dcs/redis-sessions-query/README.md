# Query DCS Redis instance client sessions

This example provides best practice code for using Terraform to query client sessions of a DCS Redis instance in
...
* `clean_cache`  - Whether to re-query and save the session list (default: false)
...
* The `node_id` is auto-obtained from the instance shards data source
* All parameters are non-updatable, changing any of them will trigger resource replacement
* Deleting this resource only removes it from Terraform state, the query is not reversed
