...
This example provides best practice code for using Terraform to create a transcoding template in HuaweiCloud MPC
(Media Processing Center) service, which defines the audio and video parameters for a single output quality.
...
* `template_name` - The name of the transcoding template
...
* `video_bitrate` - The video bitrate. 0: auto (default: 0)
* `video_fps` - The video frame rate. 0: auto (default: 0)
* `video_width` - The video width. 0: auto (default: 0)
* `video_height` - The video height. 0: auto (default: 0)
...
* The transcoding template defines output parameters for a single quality level
* For multi-quality output, use the transcoding template group instead
