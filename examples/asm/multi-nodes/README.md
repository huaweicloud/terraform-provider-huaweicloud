# Create an ASM Mesh with Multiple Nodes Configuration

This example provides best practice code for using Terraform to create an ASM (Application Service Mesh) mesh
with multiple nodes configuration for high availability and load distribution. This example demonstrates two
approaches: using existing namespaces or creating a new namespace through Terraform.
...
* Multiple nodes in the CCE cluster for installing ASM mesh components
...
This example demonstrates an ASM mesh creation workflow with multiple nodes configuration:
...
* Create a `terraform.tfvars` file and configure to create new namespace:
    ...
    namespace_name = "asm-multi-nodes-namespace"
...
* The ASM mesh components will be installed on all specified nodes using a field selector with the node UIDs.
* Specifying multiple nodes improves high availability and distributes the load across multiple nodes.
