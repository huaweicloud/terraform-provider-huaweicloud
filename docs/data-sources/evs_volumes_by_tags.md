...
* `tags` - The tag map. The [tags](#evs_volumes_tags) structure is documented below.
...
* `metadata` - The disk metadata. The [metadata](#evs_volumes_metadata) structure is documented below.
...
* `tags` - The disk tags. This field has values if the disk has tags. Or, it is left empty.
