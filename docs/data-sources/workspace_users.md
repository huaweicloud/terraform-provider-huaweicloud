...
```hcl
variable "active_type" {}

data "huaweicloud_workspace_users" "test" {
  active_type = var.active_type
}
```
...
### Compute by is query total desktops

```hcl
variable "is_query_total_desktops" {}

data "huaweicloud_workspace_users" "test" {
  is_query_total_desktops = var.is_query_total_desktops
}
```
...
* `user_name` - (Optional, String) Specifies the user name to be queried.  
  The length ranges from `1` to `20`, cannot contain special characters, and cannot start with a number.
...
* `is_query_total_desktops` - (Optional, String) Specifies whether to query the number of desktops bound to the user.  
...
* `account_expires` - The account expiration time.
