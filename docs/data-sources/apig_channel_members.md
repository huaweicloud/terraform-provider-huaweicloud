...
description: |-
  Use this data source to get the list of channel members within HuaweiCloud.
---
...
### Query channel members filter by member group name
...
* `region` - (Optional, String) Specifies the region where the list of channel members are located.  
  If omitted, the provider-level region will be used.

* `instance_id` - (Required) Specifies the ID of the dedicated instance to which the VPC channel belongs.

* `vpc_channel_id` - (Required) Specifies the ID of the VPC channel.

* `name` - (Optional) Specifies the name of the channel member for fuzzy matching.

* `member_group_name` - (Optional) Specifies the name of the channel member group for fuzzy matching.

* `member_group_id` - (Optional) Specifies the ID of the channel member group.

* `precise_search` - (Optional) Specifies the list of parameter names for exact matching.  
  When this parameter contains the field(s) to be queried, the corresponding field query will become an exact-match
  query.  
  The valid values are as follows:
  + **name**
  + **member_group_name**
  When querying multiple fields, separate them with commas(,).
