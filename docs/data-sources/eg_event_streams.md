...
* `streams` - List of event streams.  
  The [streams](#eg_event_streams_attr) structure is documented below.
...
* `created_time` - The creation time of the event stream.

* `updated_time` - The update time of the event stream.
...
* `source_type` - The type of the event source.

* `source_kafka` - The configuration of Kafka event source.

* `source_mobile_rocketmq` - The configuration of mobile cloud RocketMQ event source.

* `source_community_rocketmq` - The configuration of community RocketMQ event source.  

* `source_dms_rocketmq` - The configuration of DMS RocketMQ event source.

<a name="eg_event_streams_sink"></a>
The `sink` block supports:

* `sink_type` - The type of the event sink.

* `sink_fg` - The configuration of function graph event sink.

* `sink_kafka` - The configuration of Kafka event sink.

* `sink_obs` - The configuration of OBS event sink.

<a name="eg_event_streams_rule_config"></a>
The `rule_config` block supports:

* `transform` - The transformation rules.  
  The [transform](#eg_transform) structure is documented below.

* `filter` - The filter rules.

<a name="eg_transform"></a>
...
* `batch_window` - The batch push configuration.  
  The [batch_window](#eg_batch_window) structure is documented below.

<a name="eg_batch_window"></a>
