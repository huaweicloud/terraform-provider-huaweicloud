...
* `disconnected_retention_period` - (Optional, Int) Specifies the desktops and users disconnection retention period
  ...
  The valid value ranges from `10` to `43200`.  
  This parameter is available and required only when the the `type` is set to **DYNAMIC**.
...
* `autoscale_policy` - (Optional, List) Specifies the automatic scaling policy of the desktop pool.  
  ...
  This parameter is available only when the the `enable_autoscale` is set to **true**.
