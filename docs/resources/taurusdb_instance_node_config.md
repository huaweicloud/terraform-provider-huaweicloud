...
layout: "huaweicloud"
page_title: "HuaweiCloud: huaweicloud_taurusdb_instance_node_config"
description: |-
  Manages a TaurusDB instance node config resource within HuaweiCloud.
---

# huaweicloud_taurusdb_instance_node_config
...
* `node_id` - (Required, String, ForceNew) Specifies the ID of the TaurusDB instance node. Changing this parameter
  will create a new resource.
...
The TaurusDB instance node config can be imported using the `instance_id` and `node_id` separated by a slash, e.g.

```bash
$ terraform import huaweicloud_taurusdb_instance_node_config.test <instance_id>/<node_id>
```
