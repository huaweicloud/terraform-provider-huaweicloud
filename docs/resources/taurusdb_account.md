...
layout: "huaweicloud"
page_title: "HuaweiCloud: huaweicloud_taurusdb_account"
description: ""
---

# huaweicloud_taurusdb_account

Manages a TaurusDB account resource within HuaweiCloud.
...
* `instance_id` - (Required, String, ForceNew) Specifies the ID of the TaurusDB instance.
...
* `host` - (Optional, String, ForceNew) Specifies the host IP address. The default value is %, indicating that all IP
  addresses are allowed to access your TaurusDB instance. If its value is 10.10.10.%, all 10.10.10.X IP
  addresses can access your TaurusDB instance.
...
The TaurusDB account can be imported using the `instance_id`, `name` and `host` separated by slashes, e.g.

```bash
$ terraform import huaweicloud_taurusdb_account.test <instance_id>/<name>/<host>
```
...
MySQL account, or the resource definition should be updated to align with the TaurusDB account. Also you can
