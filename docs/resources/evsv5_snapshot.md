...
* `volume_id` - (Optional, String, NonUpdatable) Specifies the ID of the source cloud disk for the snapshot.
...
* `instant_access` - (Optional, Bool) Specifies whether to enable instant access for the snapshot. Possible values are **true** (enable) and **false** (disable). Default is **false**.

* `incremental` - (Optional, Bool, NonUpdatable) Specifies whether to create an incremental snapshot. Default is **false**.
