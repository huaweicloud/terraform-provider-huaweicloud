...
<br/>4.The upper and lower bound of the alarm interval can not be less than `0` when the
  alarm rule `condition.comparison_operator` is set to **>/>=**.
<br/>5. The upper and lower bound of the alarm interval can be less than `0` when the
  alarm rule `condition.comparison_operator` is set to **</<=**.
