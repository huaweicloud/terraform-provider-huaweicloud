...
-> This resource is only a one-time action resource using to batch update geo IP rules. Deleting this resource
  will not change the current geo IP rule configurations, but will only remove the resource information from the tf state
  file.
