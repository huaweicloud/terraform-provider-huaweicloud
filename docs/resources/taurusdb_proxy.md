...
layout: "huaweicloud"
page_title: "HuaweiCloud: huaweicloud_taurusdb_proxy"
description: |-
  Manages GaussDB mysql proxy resource within HuaweiCloud.
---

# huaweicloud_taurusdb_proxy
...
* `parameters` - (Optional, List) Specifies the list of parameters to be set to the TaurusDB proxy after launched.
...
The TaurusDB proxy can be imported using the `instance_id` and `id` separated by a slash, e.g.

```bash
$ terraform import huaweicloud_taurusdb_proxy.test <instance_id>/<id>
```
...
generally recommended running `terraform plan` after importing a TaurusDB proxy. You can then decide if changes
should be applied to the TaurusDB proxy, or the resource definition should be updated to align with the GaussDB
