...
layout: "huaweicloud"
page_title: "HuaweiCloud: huaweicloud_cce_autopilot_release"
description: |-
  Manages a CCE Autopilot release resource within HuaweiCloud.
---
...
* `region` - (Optional, String, ForceNew) Specifies the region in which to create the CCE Autopilot release resource.
  If omitted, the provider-level region will be used. Changing this creates a new CCE Autopilot release resource.

* `cluster_id` - (Required, String) Specifies the CCE Autopilot cluster ID.

* `chart_id` - (Required, String) Specifies the CCE Autopilot chart ID.
...
```bash
$ terraform import huaweicloud_cce_autopilot_release.test <cluster_id>/<namespace>/<chart_name>
```
