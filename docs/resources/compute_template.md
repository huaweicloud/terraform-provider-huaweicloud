...
API response. The missing attribute is: `name`, and `description`. It is generally recommended running `terraform plan`
after importing an ECS template. You can then decide if changes should be applied to the ECS template, or the resource
definition should be updated to align with the ECS template. Also you can ignore changes as below.
