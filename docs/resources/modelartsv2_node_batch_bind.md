...
* `pool_id` - (Required, String, NonUpdatable) Specifies the ID of the resource pool to which the nodes belong.

* `nodes` - (Required, List, NonUpdatable) Specifies the list of nodes to be bound to the logical sub-pool.
  The [nodes](#NodeBatchBindNodes) structure is documented below.

* `drain` - (Optional, Bool, NonUpdatable) Specifies whether to drain the nodes during the bind operation.

<a name="NodeBatchBindNodes"></a>
The `nodes` block supports:

* `name` - (Required, String, NonUpdatable) Specifies the name of the node to be bound.

* `quota_name` - (Optional, String, NonUpdatable) Specifies the ID of the logical sub-pool to which the node is bound.
