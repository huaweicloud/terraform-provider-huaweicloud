...
* `cluster_name` - (Required, String) Specifies the CCE cluster name.

* `auto_upgrade` - (Required, Bool) Specifies whether to enable automatic agent upgrade.

* `runtime_info` - (Required, List) Specifies the container runtime configuration.
...
-> `invoked_service`, `charging_mode`, `cce_protection_type`, and `prefer_packet_cycle` parameters are used in the CCE
  integrated protection call scenario.
...
* `runtime_name` - (Required, String) Specifies the runtime name.  
  The valid values are as follows:
  + **crio_endpoint**: CRIO.
...
* `runtime_path` - (Optional, String) Specifies the runtime path.
...
API response, security or some other reason. The missing attributes include: `invoked_service`, `agent_version`,
`charging_mode`, `cce_protection_type`, `prefer_packet_cycle`, `enterprise_project_id`.
...
resource "huaweicloud_hss_container_kubernetes_cluster_daemonset" "test" { 
  ...
  lifecycle {
    ignore_changes = [
      invoked_service, agent_version, charging_mode, cce_protection_type, prefer_packet_cycle, enterprise_project_id,
    ]
  }
}
```
