...
* `topic_urn` - (Required, String) Specifies the resource identifier of a topic.

* `subject` - (Optional, String) Specifies the message title.

* `message` - (Optional, String) Specifies the message content.

* `message_attributes` - (Optional, List) Specifies the message filter policies of a subscriber.
  The [message_attributes](#smn_message_attributes) structure is documented below.

* `message_structure` - (Optional, String) Specifies the message structure.

* `message_template_name` - (Optional, String) Specifies the message template name.

* `time_to_live` - (Optional, String) Specifies the maximum retention time of the message within the SMN system.
...
* `tags` - (Optional, Map) Specifies a dictionary consisting of tag and parameters to replace the tag.
...
* `name` - (Required, String) Specifies the property name.

* `type` - (Required, String) Specifies the property type.
  The value can be **STRING**, **STRING_ARRAY** or **PROTOCOL**.

* `value` - (Optional, String) Specifies the property value.
  Fill in this field value when the attribute type is only **STRING**, the attribute value can only contain Chinese
  and English, numbers, and underscores, and the length is **1** to **32** characters.

* `values` - (Optional, List) Specifies the property values.
  + When the attribute type is **STRING_ARRAY**, the attribute value is a string array, the array length is
  ...
  + When the attribute type is **PROTOCOL**, the attribute value is a string array of supported protocol types.
