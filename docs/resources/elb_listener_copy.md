...
Manages an ELB listener copy resource within HuaweiCloud.
...
* `loadbalancer_id` - (Required, String, NonUpdatable) Specifies the load balancer on which to provision this listener.

* `name` - (Optional, String) Specifies the name of the new listener.

* `protocol_port` - (Optional, Int, NonUpdatable) Specifies the port used by the listener.
...
* `port_ranges` - (Optional, List, NonUpdatable) Specifies the port monitoring range (closed range), specify up to 10 port
...
* `reuse_pool` - (Optional, String) Specifies whether to reuse or copy the backend server groups and backend servers of
  the original listener.
  + **Reuse**: The destination load balancer directly uses the backend server group of the original listener.
  + **Copy**: New backend server groups with the same settings as those of the original listener will be created and
    associated with the destination load balancer.
  If this parameter is set to **true**, the backend server groups of the original listener must be associated with
  multiple instances. This parameter is available only for copying a listener from a dedicated load balancer to another
  in the same VPC. Value options:
  + **true**: The backend server groups of the original listener will be used.
  + **false**: New backend server groups with the same settings as those of the original listener will be created and
    associated with the destination load balancer.
...
ELB listener copy can be imported using the listener ID, e.g.
...
API response, security or some other reason. The missing attributes include: `force_delete`, `listener_id`, `reuse_pool`.
It is generally recommended running `terraform plan` after importing a listener copy. You can then decide if changes
should be applied to the listener copy, or the resource definition should be updated to align with the listener copy.
...
resource "huaweicloud_elb_listener_copy" "test" {
  ...
  lifecycle {
    ignore_changes = [
      force_delete, listener_id, reuse_pool
    ]
  }
}
```
