...
* `account_name` - (Required, String, NonUpdatable) Specifies the account name to which the IAM user belongs.
  Changing this will create a new token.

* `user_name` - (Required, String, NonUpdatable) Specifies the IAM user name. Changing this will create a new token.

* `password` - (Required, String, NonUpdatable) Specifies the IAM user password. Changing this will create a new token.

* `project_name` - (Optional, String, NonUpdatable) Specifies the project name. If it is blank, the token applies to
...
* `token` - The token of the IAM user. The valid period is `24` hours.
