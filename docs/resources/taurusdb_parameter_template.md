...
layout: "huaweicloud"
page_title: "HuaweiCloud: huaweicloud_taurusdb_parameter_template"
description: |-
  Manages a TaurusDB parameter template resource within HuaweiCloud.
---
...
* `instance_id` - (Optional, String, ForceNew) Specifies the ID of the TaurusDB instance.

  Changing this parameter will create a new resource.

* `instance_configuration_id` - (Optional, String, ForceNew) Specifies the parameter template ID of the TaurusDB
  instance.
...
-> **NOTE:** 1. At most one of `datastore_engine`, `source_configuration_id` and `instance_id` can be specified.
  ...
  <br>3. If `instance_id` is specified, then the resource will replicate the parameter template of the TaurusDB
...
```bash
$ terraform import huaweicloud_taurusdb_parameter_template.test <id>
```
...
and `instance_configuration_id`. It is generally recommended running `terraform plan` after importing an TaurusDB
parameter template. You can then decide if changes should be applied to the TaurusDB parameter template, or the
TaurusDB parameter template definition should be updated to align with the instance. Also, you can ignore changes
as below.
