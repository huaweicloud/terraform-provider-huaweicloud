...
* `waf_failure_action` - (Optional, String) Specifies traffic distributing policies when the WAF is faulty.
  ...
  + **false**: Traffic will be distributed to the default backend servers.
...
* `listeners` - Indicates the list of the IDs of listeners added to the load balancer.
  The [listeners](#elb_listeners_pools) structure is documented below.

* `pools` - Indicates the list of the IDs of backend server groups associated with the load balancer.
  The [pools](#elb_listeners_pools) structure is documented below.
...
<a name="elb_listeners_pools"></a>
The `elb_listeners_pools` block supports:

* `id` - Indicates the listener/pool ID of the ELB instance.
