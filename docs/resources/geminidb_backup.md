...
The GeminiDB backup can be imported using the `id`, e.g.

```bash
$ terraform import huaweicloud_geminidb_backup.test <id>
```

Note that the imported state may not be identical to your resource definition, due to some attributes missing from the
API response. The missing attributes include: `database_tables` .
It is generally recommended running `terraform plan` after importing an backup.
You can then decide if changes should be applied to the backup, or the resource definition should be updated to
align with the backup. Also you can ignore changes as below.
