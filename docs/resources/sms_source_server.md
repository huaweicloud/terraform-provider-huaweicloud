...
* `datasource_source_id` - (Optional, String) Specifies the ID of the source server in the SMS database.
...
* `hostname` - (Optional, String) Specifies the hostname of the source server.

  -> This parameter is mandatory for registering the source server with SMS and optional for updating the information
  about the source server.

* `os_type` - (Optional, String) Specifies the OS type of the source server, which can be Windows or Linux.
  Values can be **Windows** and **Linux**.

  -> This parameter is mandatory for registering the source server with SMS and optional for updating the information
  about the source server.

* `os_version` - (Optional, String) Specifies the OS version.

  -> This parameter is mandatory for registering the source server with SMS and optional for updating the information
  about the source server.

* `virtualization_type` - (Optional, String) Specifies the OS virtualization type.

* `linux_block_check` - (Optional, String) Specifies the Linux block-level check.

* `firmware` - (Optional, String) Specifies the boot mode.
  Values can be **BIOS** and **UEFI**.

* `cpu_quantity` - (Optional, Int) Specifies the number of CPUs, the unit is vCPUs.

* `memory` - (Optional, Int) Specifies the memory size, the unit is MB.
...
* `domain_id` - (Optional, String) Specifies the tenant domain ID.

* `has_rsync` - (Optional, Bool) Specifies whether rsync is installed.

  -> This parameter is mandatory for Linux.
* `paravirtualization` - (Optional, Bool) Specifies whether the source server is paravirtualized.

  -> This parameter is mandatory for Linux.

* `raw_devices` - (Optional, String) Specifies the list of raw devices.

  -> This parameter is mandatory for Linux.

* `driver_files` - (Optional, Bool) Specifies whether any driver files are missing.

  -> This parameter is mandatory for Windows.

* `system_services` - (Optional, Bool) Specifies whether there are abnormal services.

  -> This parameter is mandatory for Windows.

* `account_rights` - (Optional, Bool) Specifies whether the account has the required permissions.

  -> This parameter is mandatory for Windows.

* `boot_loader` - (Optional, String) Specifies the system boot loader.
  Values can be **GRUB** and **LILO**.

  -> This parameter is mandatory for Linux.

* `system_dir` - (Optional, String) Specifies the system directory.
...
* `kernel_version` - (Optional, String) Specifies the kernel version.
...
* `oem_system` - (Optional, Bool) Specifies whether the OS is an OEM version (Windows).

* `start_type` - (Optional, String) Specifies the startup mode.
  Values can be **MANUAL**, **MGC** or an empty string ("").

* `io_read_wait` - (Optional, Int) Specifies the disk read latency, the unit is ms.

* `has_tc` - (Optional, Bool) Specifies whether TC is installed.

  -> This parameter is mandatory for Linux.

* `platform` - (Optional, String) Specifies the platform.
  Values can be as follows:
  + **hw**: Huawei Cloud.
  + **ali**: Alibaba Cloud.
  + **aws**: AWS.
...
* `name` - (Required, String) Specifies the NIC name.

* `ip` - (Required, String) Specifies the IP address bound to the NIC.

* `netmask` - (Required, String) Specifies the subnet mask.

* `gateway` - (Required, String) Specifies the gateway.

* `mac` - (Required, String) Specifies the MAC address.

* `ipv6` - (Optional, String) Specifies the IPv6 address.

* `mtu` - (Optional, Int) Specifies the NIC MTU.
