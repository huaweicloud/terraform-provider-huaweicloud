...
* `enable_approve` - (Required, Bool) Specifies whether to enable manual review of incoming shipments.

* `enable_message_notification` - (Required, Bool) Specifies whether to enable message notifications.

* `ticket_infos` - (Optional, List) Specifies the order No. 4 rights request information.
...
* `enabled` - (Optional, Bool) Specifies whether the task is enabled. The default value is **true**.
...
* `period` - (Optional, String) Specifies the specific week list for periodic scheduled tasks.
  ...
  and Sunday, the value would be **1,2,4,5**. The default value is *.
...
* `cron` - (Optional, String) Specifies the specific CRON expression value for executing a scheduled task based on a
  ...
  For example, to execute a task at 10:15 AM every day, use ```15 10 ? * *```.
...
* `order_no` - (Required, Int) Specifies the step number.
  ...
  -> **NOTE:** When a scheduled task is associated with a script, this value is **1**.
  <br>When a scheduled task is associated with a job, this value indicates the step number within the job.
...
* `type` - (Optional, String) Specifies the resource selection method (REGION, APPLICATION). The default value is **REGION**.
  Values can be as follows:
...
* `app_name` - (Optional, String) Specifies the application name (separate hierarchical elements with .).
...
* `ticket_id` - (Optional, String) Specifies the ID of order No. 4.

* `ticket_type` - (Optional, String) Specifies the type of order No. 4.
  Values can be as follows:
  + **CHANGE**: Change order.
  + **INCIDENT**: Incident order.
  + **ALARM**: Alarm order.
  + **WARROOM**: War room order.
* `target_id` - (Optional, String) Specifies the application ID associated with the order No. 4.

* `scope_id` - (Optional, String) Specifies the region ID associated with the order No. 4.

<a name="message_notification_struct"></a>
The `reviewer_notification` and `message_notification` block supports:

* `notification_endpoint_type` - (Required, String) Specifies the notification policy.
  Values can be as follows:
...
* `policy` - (Optional, String) Specifies the notification recipient type. The default value is **ONCALL**.
  Values can be as follows:
  + **USER**: Individual.
  + **ONCALL**: Scheduled.
