...
* `termination_grace_period_seconds` - The restart policy for all containers within the pod.
...
* `preferred_during_scheduling_ignored_during_execution` - The preferred during scheduling
  ignored during execution.
  The [preferred_during_scheduling_ignored_during_execution](#pods_affinity_pod_anti_affinity_preferred) structure is
  documented below.

* `required_during_scheduling_ignored_during_execution` - The required during schedulin
  ignored during execution.
...
* `pod_affinity_term` - The pod affinity term.
  The [pod_affinity_term](#pods_affinity_pod_anti_affinity_required_pod_affinity_term) structure is documented below.

* `weight` - The weight.

<a name="pods_affinity_pod_anti_affinity_required_pod_affinity_term"></a>
The `pod_affinity_term` block supports:

* `label_selector` - The label selector.
  The [label_selector](#pods_affinity_pod_anti_affinity_required_pod_affinity_term_label_selector) structure is
  documented below.
...
<a name="pods_affinity_pod_anti_affinity_required_pod_affinity_term_label_selector"></a>
The `topology_key` block supports:

* `match_expressions` - The match expressions.
  The [match_expressions](#pods_affinity_pod_anti_affinity_required_pod_affinity_term_topology_key_match_expressions)
  structure is documented below.

* `match_labels` - The match labels.

<a name="pods_affinity_pod_anti_affinity_required_pod_affinity_term_topology_key_match_expressions"></a>
...
* `exec` - The pre stop of the lifecycle.
  The [exec](#pods_containers_lifecycle_pre_stop_exec) structure is documented below.

* `http_get` - The http get of the pre stop.
  The [http_get](#pods_containers_lifecycle_pre_stop_http_get) structure is documented below.

<a name="pods_containers_lifecycle_pre_stop_exec"></a>
The `exec` block supports:

* `command` - The command of the exec.

<a name="pods_containers_lifecycle_pre_stop_http_get"></a>
The `http_get` block supports:

* `host` - The host of the http get.

* `http_headers` - The http headers of the http headers.
  The [http_headers](#pods_containers_lifecycle_pre_stop_http_get_http_headers) structure is documented below.
...
<a name="pods_containers_lifecycle_pre_stop_http_get_http_headers"></a>
...
* `proc_mount` - The denotes the type of proc mount to use for the containers.
...
The `secret_name` block supports:
