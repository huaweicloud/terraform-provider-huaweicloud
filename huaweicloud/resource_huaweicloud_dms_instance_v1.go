...
func resourceDmsInstancesV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	if resourceTags, err := tags.Get(dmsV2Client, engine, d.Id()).Extract(); err == nil {
		...
		if err := d.Set("tags", tagmap); err != nil {
			return fmt.Errorf("Error saving tags to state for HuaweiCloud DMS instance (%s): %s", d.Id(), err)
		}
	} else {
		log.Printf("[WARN] Error fetching tags of HuaweiCloud DMS instance (%s): %s", d.Id(), err)
	}
