...
func TestAccV3VirtualMfaDevices_basic(t *testing.T) {
	var (
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccV3VirtualMfaDevices_basic(name),
...
func testAccV3VirtualMfaDevices_basic(name string) string {
	return fmt.Sprintf(`
%[1]s
...
`, testAccV3VirtualMFADevice_basic(name), acceptance.HW_USER_ID)
}
