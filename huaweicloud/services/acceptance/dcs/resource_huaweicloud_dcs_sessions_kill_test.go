...
func TestAccDcsSessionsKill_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDcsSessionsKill_basic(name),
			},
		},
	})
}

func testAccDcsSessionsKill_base(name string) string {
	return fmt.Sprintf(`
...
func testAccDcsSessionsKill_basic(name string) string {
	return fmt.Sprintf(`
%s
...
resource "huaweicloud_dcs_sessions_kill" "test" {
  ...
  client_addrs   = ["127.0.0.1:6379"]
}
`, testAccDcsSessionsKill_base(name))
}
