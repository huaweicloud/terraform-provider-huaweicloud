...
func TestAccDcsOnlineDataMigrationTaskRestart_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDcsMigrationTaskRestart_basic(name),
			},
		},
	})
}

func testDcsMigrationTaskRestart_base(name string) string {
	return fmt.Sprintf(`
...
func testDcsMigrationTaskRestart_basic(name string) string {
	return fmt.Sprintf(`
%s

resource "huaweicloud_dcs_online_data_migration_task_restart" "test" {
  depends_on = [huaweicloud_dcs_migration_task_stop.test]

  task_id = huaweicloud_dcs_online_data_migration_task.test.id
}
`, testDcsMigrationTaskRestart_base(name))
}
