		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(dataSource, "result.#", regexp.MustCompile(`^[1-9]([0-9]*)?$`)),
					...
					resource.TestCheckResourceAttrSet(byIds, "result.0.host_type"),
					resource.TestMatchResourceAttr(byIds, "result.0.updated_at",
...
locals {
  ids_filter_result = [
    for v in data.huaweicloud_lts_hosts.filter_by_ids.result[*].host_id : v if contains(local.host_ids, v)
  ]
}
...
# Filter by host name
locals {
  host_names = data.huaweicloud_lts_hosts.filter_by_ids.result[*].host_name
}
...
locals {
  names_filter_result = [
    for v in data.huaweicloud_lts_hosts.filter_by_names.result[*].host_name : v if contains(local.host_names, v)
  ]
}
...
# Filter by host IP
locals {
  host_ips = data.huaweicloud_lts_hosts.filter_by_ids.result[*].host_ip
}
...
locals {
  ips_filter_result = [
    for v in data.huaweicloud_lts_hosts.filter_by_ips.result[*].host_ip : v if contains(local.host_ips, v)
  ]
}
...
locals {
  host_status = try(data.huaweicloud_lts_hosts.test.result[0].host_status, "")
}
...
locals {
  status_filter_result = [
    for v in data.huaweicloud_lts_hosts.filter_by_status.result[*].host_status : v == local.host_status
  ]
}
...
locals {
  host_version = try(data.huaweicloud_lts_hosts.test.result[0].host_version, "")
}
...
locals {
  version_filter_result = [
    for v in data.huaweicloud_lts_hosts.filter_by_version.result[*].host_version : v == local.host_version
  ]
}
