...
func TestAccDataSourceRulesGeolocationAccessControl_basic(t *testing.T) {
	var (
		...
		byRuleId   = "data.huaweicloud_waf_rules_geolocation_access_control.filter_by_rule_id"
		dcByRuleId = acceptance.InitDataSourceCheck(byRuleId)

		byName           = "data.huaweicloud_waf_rules_geolocation_access_control.filter_by_rule_id"
		dcByName         = acceptance.InitDataSourceCheck(byName)
		byNameNotFound   = "data.huaweicloud_waf_rules_geolocation_access_control.not_found"
		dcByNameNotFound = acceptance.InitDataSourceCheck(byNameNotFound)

		byStatus   = "data.huaweicloud_waf_rules_geolocation_access_control.filter_by_status"
		dcByStatus = acceptance.InitDataSourceCheck(byStatus)

		byAction   = "data.huaweicloud_waf_rules_geolocation_access_control.filter_by_action"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceWafRulesGeolocationAccessControl_basic(rName),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(dataSourceName, "rules.0.geolocation"),
...
func testDataSourceWafRulesGeolocationAccessControl_basic(name string) string {
	return fmt.Sprintf(`
%[1]s
...
locals {
  rule_id = data.huaweicloud_waf_rules_geolocation_access_control.test.rules[0].id
}
...
locals {
  rule_id_filter_result = [
    for v in data.huaweicloud_waf_rules_geolocation_access_control.filter_by_rule_id.rules[*].id : v == local.rule_id
  ]
}
...
locals {
  name = data.huaweicloud_waf_rules_geolocation_access_control.test.rules[0].name
}
...
locals {
  name_filter_result = [
    for v in data.huaweicloud_waf_rules_geolocation_access_control.filter_by_name.rules[*].name : v == local.name
  ]
}
...
output "is_not_found" {
  value = length(data.huaweicloud_waf_rules_geolocation_access_control.not_found.rules) == 0
}

locals {
  status = data.huaweicloud_waf_rules_geolocation_access_control.test.rules[0].status
}
...
locals {
  status_filter_result = [ 
    for v in data.huaweicloud_waf_rules_geolocation_access_control.filter_by_status.rules[*].status : v == local.status
  ]
}
...
locals {
  action = data.huaweicloud_waf_rules_geolocation_access_control.test.rules[0].action
}
...
locals {
  action_filter_result = [ 
    for v in data.huaweicloud_waf_rules_geolocation_access_control.filter_by_action.rules[*].action : v == local.action
  ]
}
