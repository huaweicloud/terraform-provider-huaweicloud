...
func TestAccDataSourceAllPreciseProtectionRules_basic(t *testing.T) {
	var (
		dataSourceName = "data.huaweicloud_waf_all_precise_protection_rules.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			// Prepare a WAF precise protection rule before test
			acceptance.TestAccPrecheckWafInstance(t)
		},
		ProviderFactories: acceptance.TestAccProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceAllPreciseProtectionRules_basic,
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(dataSourceName, "items.0.priority"),
					resource.TestCheckResourceAttrSet(dataSourceName, "items.0.timestamp"),
					resource.TestCheckResourceAttrSet(dataSourceName, "items.0.time"),
...
const testAccDataSourceAllPreciseProtectionRules_basic = `
data "huaweicloud_waf_all_precise_protection_rules" "test" {}

locals {
  policyid = data.huaweicloud_waf_all_precise_protection_rules.test.items[0].policyid
}

data "huaweicloud_waf_all_precise_protection_rules" "filter_by_policyid" {
  policyids = local.policyid
}

output "policyid_filter_is_useful" {
  value = length(data.huaweicloud_waf_all_precise_protection_rules.filter_by_policyid.items) > 0 && alltrue(
    [for v in data.huaweicloud_waf_all_precise_protection_rules.filter_by_policyid.items[*].policyid : v == local.policyid]
  )
}

data "huaweicloud_waf_all_precise_protection_rules" "filter_by_eps" {
  enterprise_project_id = "0"
}

output "eps_filter_is_useful" {
  value = length(data.huaweicloud_waf_all_precise_protection_rules.filter_by_eps.items) > 0
}
`
