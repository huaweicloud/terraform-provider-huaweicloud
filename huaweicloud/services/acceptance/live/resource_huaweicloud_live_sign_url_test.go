...
func TestAccSignUrl_basic(t *testing.T) {
	var (
		rName1           = "huaweicloud_live_sign_url.test"
		rName2           = "huaweicloud_live_sign_url.try"
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccSignUrl_ingest(ingestDomainName, startTime),
			...
			{
				// The action resource do not need to be checked and no processing is performed in the Read method.
				Config: testAccSignUrl_stream(streamDomainName),
				Check: resource.ComposeTestCheckFunc(
...
func testAccSignUrl_ingest(name, startTime string) string {
	return fmt.Sprintf(`
...
func testAccSignUrl_stream(name string) string {
	return fmt.Sprintf(`
...
resource "huaweicloud_live_sign_url" "try" {
  domain_name = huaweicloud_live_domain.try.name
  type        = "pull"
  app_name    = "live2"
  stream_name = "tf-try"
