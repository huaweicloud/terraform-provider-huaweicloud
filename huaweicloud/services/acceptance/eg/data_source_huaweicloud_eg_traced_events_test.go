...
func testAccDataSourceTracedEvents_base(name, eventType string) string {
...
resource "huaweicloud_eg_event_subscription" "test" {
  ...
  targets {
    ...
    detail_name   = "detail"
    detail        = jsonencode({
      "urn":"urn:fss:%[3]s:%[4]s:function:default:%[5]s:latest",
      "invoke_type":"ASYNC",
...
`, name, targetID, acceptance.HW_REGION_NAME, acceptance.HW_PROJECT_ID, acceptance.HW_FG_FUNCTION_NAME, eventType)
}
...
`, testAccDataSourceTracedEvents_base(name, eventType), startTime, endTime, name, eventType)
}
