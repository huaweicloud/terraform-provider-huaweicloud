...
func TestAccDdmInstanceUpgrade_basic(t *testing.T) {
	var obj interface{}
	rName := acceptance.RandomAccResourceNameWithDash()
	resourceName := "huaweicloud_ddm_instance_upgrade.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDdmInstanceUpgrade_basic(rName),
...
func testAccDdmInstanceUpgrade_base(rName string) string {
	return fmt.Sprintf(`
%[1]s
...
func testAccDdmInstanceUpgrade_basic(rName string) string {
	return fmt.Sprintf(`
%s
...
}`, testAccDdmInstanceUpgrade_base(rName))
}
