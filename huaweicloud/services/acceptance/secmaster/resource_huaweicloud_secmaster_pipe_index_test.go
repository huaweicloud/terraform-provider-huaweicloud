...
func getResourcePipeIndexFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	return secmaster.GetPipeIndexDetail(client, state.Primary.Attributes["workspace_id"], state.Primary.ID)
}

func TestAccResourcePipeIndex_basic(t *testing.T) {
	var (
		rName = "huaweicloud_secmaster_pipe_index.test"

		object interface{}
		rc     = acceptance.InitResourceCheck(
			rName,
			&object,
			getResourcePipeIndexFunc,
		)
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "timestamp_field", "event_time"),
				),
			},
			{
				Config: testAccPipeIndex_update(),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "workspace_id", acceptance.HW_SECMASTER_WORKSPACE_ID),
					resource.TestCheckResourceAttr(rName, "pipe_id", acceptance.HW_SECMASTER_PIPE_ID),
					resource.TestCheckResourceAttrSet(rName, "mapping"),
					resource.TestCheckResourceAttr(rName, "timestamp_field", "update_event_time"),
				),
			},
			{
				ResourceName:      rName,
				...
				ImportStateIdFunc: testAccPipeIndexImportStateFunc(rName),
			},
		},
	})
}

func testAccPipeIndex_basic() string {
	return fmt.Sprintf(`
...
func testAccPipeIndex_update() string {
	return fmt.Sprintf(`
resource "huaweicloud_secmaster_pipe_index" "test" {
  workspace_id    = "%[1]s"
  pipe_id         = "%[2]s"
  ...
  timestamp_field = "update_event_time"
}
`, acceptance.HW_SECMASTER_WORKSPACE_ID, acceptance.HW_SECMASTER_PIPE_ID)
}

func testAccPipeIndexImportStateFunc(rName string) resource.ImportStateIdFunc {
