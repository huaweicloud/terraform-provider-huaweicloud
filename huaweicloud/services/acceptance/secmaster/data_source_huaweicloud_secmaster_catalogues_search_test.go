		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckOutput("is_parent_catalogue_filter_useful", "true"),
...
# Filter by parent_catalogue
locals {
  parent_catalogue = data.huaweicloud_secmaster_catalogues_search.test.data[0].parent_catalogue
}

data "huaweicloud_secmaster_catalogues_search" "filter_by_parent_catalogue" {
  workspace_id     = "%[1]s"
  parent_catalogue = local.parent_catalogue
}

locals {
  list_by_parent_catalogue = data.huaweicloud_secmaster_catalogues_search.filter_by_parent_catalogue.data
}

output "is_parent_catalogue_filter_useful" {
  value = length(local.list_by_parent_catalogue) > 0 && alltrue(
    [for v in local.list_by_parent_catalogue[*].parent_catalogue : v == local.parent_catalogue]
  )
}
