...
func TestAccDataSourceRmsProjectTags_basic(t *testing.T) {
	dataSource := "data.huaweicloud_rms_project_tags.test"
	dc := acceptance.InitDataSourceCheck(dataSource)
	name := acceptance.RandomAccResourceName()
	baseConfig := testDataSourceProjectTags_base(name)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceProjectTags_basic(baseConfig),
...
func testDataSourceProjectTags_basic(baseConfig string) string {
	return fmt.Sprintf(`
%[1]s
...
func testDataSourceProjectTags_base(name string) string {
	return fmt.Sprintf(`
