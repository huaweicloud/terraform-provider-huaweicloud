...
func TestAccDeadLockSwitch_basic(t *testing.T) {
	var (
		rName = "huaweicloud_das_dead_lock_switch.test"
	)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDeadLockSwitch_basic(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(rName, "switch_on", "false"),
				),
			},
			{
				Config: testAccDeadLockSwitch_basic_update(),
				Check: resource.ComposeTestCheckFunc(
...
func testAccDeadLockSwitch_base() string {
	return fmt.Sprintf(`
locals {
...
func testAccDeadLockSwitch_basic() string {
	return fmt.Sprintf(`
%[1]s
...
`, testAccDeadLockSwitch_base())
}

func testAccDeadLockSwitch_basic_update() string {
	return fmt.Sprintf(`
%[1]s
