...
func TestAccEmailBatchSubscription_basic(t *testing.T) {
	var (
		rName = "huaweicloud_das_email_batch_subscription.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccEmailBatchSubscription_basic(name),
			...
			{
				Config: testAccEmailBatchSubscription_basic_update(name),
				Check: resource.ComposeTestCheckFunc(
...
func testAccEmailBatchSubscription_base(name string) string {
	return fmt.Sprintf(`
...
func testAccEmailBatchSubscription_basic(name string) string {
	return fmt.Sprintf(`
%[1]s
...
`, testAccEmailBatchSubscription_base(name))
}

func testAccEmailBatchSubscription_basic_update(name string) string {
	return fmt.Sprintf(`
%[1]s
