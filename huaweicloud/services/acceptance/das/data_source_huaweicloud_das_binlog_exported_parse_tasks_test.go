...
func TestAccDataBinlogExportedParseTasks_basic(t *testing.T) {
	var (
		all = "data.huaweicloud_das_binlog_exported_parse_tasks.all"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataBinlogExportedParseTasks_basic(name),
...
func testAccDataBinlogExportedParseTasks_base(name string) string {
	return fmt.Sprintf(`
...
func testAccDataBinlogExportedParseTasks_basic(name string) string {
	return fmt.Sprintf(`
%[1]s
...
data "huaweicloud_das_binlog_exported_parse_tasks" "all" {
  user_id = local.user_id
...
`, testAccDataBinlogExportedParseTasks_base(name))
}
