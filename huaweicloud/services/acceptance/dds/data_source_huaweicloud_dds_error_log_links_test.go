...
func TestAccDataSourceDDSErrorLogLinks_basic(t *testing.T) {
	dataSource := "data.huaweicloud_dds_error_log_links.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceDDSErrorLogLinks_basic(),
...
func testDataSourceDDSErrorLogLinks_basic() string {
	return fmt.Sprintf(`
...
locals {
  file_name = data.huaweicloud_dds_error_log_links.test.links.0.file_name
}

data "huaweicloud_dds_error_log_links" "filter" {
  instance_id    = "%[1]s"
  file_name_list = [local.file_name]
}

output "file_name_filter_is_useful" {
  value = length(data.huaweicloud_dds_error_log_links.filter.links) > 0 && alltrue(
    [for v in data.huaweicloud_dds_error_log_links.filter.links[*].file_name : v == local.file_name]
  )
