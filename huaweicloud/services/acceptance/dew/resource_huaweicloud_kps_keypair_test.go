...
func TestAccKpsKeypair_privateKey(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testKeypair_ClearPrivateKey(rName),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", rName),
					resource.TestCheckResourceAttr(resourceName, "public_key", publicKeyValue),
			...
			{
				Config: testKeypair_ImportPrivateKey(rName),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", rName),
					resource.TestCheckResourceAttr(resourceName, "public_key", publicKeyValue),
...
func testKeypair_ClearPrivateKey(rName string) string {
	return fmt.Sprintf(`
resource "huaweicloud_kps_keypair" "test" {
  name = "%[1]s"
}
...
func testKeypair_ImportPrivateKey(rName string) string {
	return fmt.Sprintf(`
