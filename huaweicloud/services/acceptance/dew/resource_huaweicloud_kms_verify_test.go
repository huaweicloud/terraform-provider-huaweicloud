...
func TestAccKmsVerify_basic(t *testing.T) {
	// lintignore:AT001
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccKmsVerify_basic(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("huaweicloud_kms_verify.test", "key_id", acceptance.HW_KMS_KEY_ID),
...
func testAccKmsVerify_basic() string {
	return fmt.Sprintf(`
%[1]s
...
output verify_validation {
	value = huaweicloud_kms_verify.test.signature_valid
}
