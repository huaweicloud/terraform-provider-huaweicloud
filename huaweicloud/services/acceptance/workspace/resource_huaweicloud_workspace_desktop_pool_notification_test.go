...
func TestAccDesktopPoolNotification_basic(t *testing.T) {
	var (
		name         = acceptance.RandomAccResourceNameWithDash()
		resourceName = "huaweicloud_workspace_desktop_pool_notification.test"
	)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDesktopPoolNotification_basic(name),
...
func testAccDesktopPoolNotification_basic(name string) string {
	return fmt.Sprintf(`
%[1]s
...
`, testAccDesktopPoolNotification_base(name))
}

func testAccDesktopPoolNotification_base(name string) string {
	return fmt.Sprintf(`
