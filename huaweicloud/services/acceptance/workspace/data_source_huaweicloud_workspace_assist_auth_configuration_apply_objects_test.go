...
func TestAccDataAssistAuthConfigurationApplyObjects_basic(t *testing.T) {
	var (
		name = acceptance.RandomAccResourceName()

		all = "data.huaweicloud_workspace_assist_auth_configuration_apply_objects.all"
		dc  = acceptance.InitDataSourceCheck(all)

		filterByObjectTypeUser        = "data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_type_user"
		dcFilterByObjectTypeUser      = acceptance.InitDataSourceCheck(filterByObjectTypeUser)
		filterByObjectTypeUserGroup   = "data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_type_user_group"
		dcFilterByObjectTypeUserGroup = acceptance.InitDataSourceCheck(filterByObjectTypeUserGroup)

		filterByObjectName   = "data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_name"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataAssistAuthConfigurationApplyObjects_basic(name),
...
func testAccDataAssistAuthConfigurationApplyObjects_base(name string) string {
	return fmt.Sprintf(`
...
func testAccDataAssistAuthConfigurationApplyObjects_basic(name string) string {
	return fmt.Sprintf(`
%[1]s

# Without any filter parameter.
data "huaweicloud_workspace_assist_auth_configuration_apply_objects" "all" {
  depends_on = [
    huaweicloud_workspace_assist_auth_configuration_object_batch_apply.test,
  ]
}

# Filter by 'object_type' parameter and the value is 'USER'.
data "huaweicloud_workspace_assist_auth_configuration_apply_objects" "filter_by_object_type_user" {
  depends_on = [
    huaweicloud_workspace_assist_auth_configuration_object_batch_apply.test,
  ]
...
locals {
  object_type_user_filter_result = [
    for v in data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_type_user.objects[*].type :
    v == "USER"
  ]
}
...
# Filter by 'object_type' parameter and the value is 'USER_GROUP'.
data "huaweicloud_workspace_assist_auth_configuration_apply_objects" "filter_by_object_type_user_group" {
  depends_on = [
    huaweicloud_workspace_assist_auth_configuration_object_batch_apply.test,
  ]
...
locals {
  object_type_user_group_filter_result = [
    for v in data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_type_user_group.objects[*].type :
    v == "USER_GROUP"
  ]
}
...
# Filter by 'object_name' parameter.
locals {
  object_name = try(data.huaweicloud_workspace_assist_auth_configuration_apply_objects.all.objects[0].name, "NOT_FOUND")
}

data "huaweicloud_workspace_assist_auth_configuration_apply_objects" "filter_by_object_name" {
  depends_on = [
    huaweicloud_workspace_assist_auth_configuration_object_batch_apply.test,
  ]
...
locals {
  object_name_filter_result = [
    for v in data.huaweicloud_workspace_assist_auth_configuration_apply_objects.filter_by_object_name.objects[*].name :
    v == local.object_name
  ]
}
...
`, testAccDataAssistAuthConfigurationApplyObjects_base(name), name)
}
