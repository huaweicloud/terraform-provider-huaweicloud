...
func getResourceAppRuleFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	return workspace.GetAppRule(client, state.Primary.Attributes["name"], state.Primary.ID)
}
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(withProductRule, "description", "Updated by terraform script"),
					resource.TestCheckResourceAttr(withProductRule, "rule.0.path_rule.#", "0"),
					rcWithPath.CheckResourceExists(),
					resource.TestCheckResourceAttr(withPathRule, "name", name+"_path"),
					resource.TestCheckResourceAttr(withPathRule, "rule.0.scope", "PATH"),
			...
			{
				Config:      testAccResourceAppRule_validation_missing_identify_condition(name),
				ExpectError: regexp.MustCompile(`The argument "identify_condition" is required, but no definition was found.`),
			},
			{
				Config:      testAccResourceAppRule_validation_all_star_fields(name),
				ExpectError: regexp.MustCompile(`publisher, product_name, and process_name cannot be all '\*' or empty in product_rule`),
			},
		},
	})
}
...
func testAccResourceAppRule_validation_missing_identify_condition(name string) string {
	return fmt.Sprintf(`
resource "huaweicloud_workspace_app_rule" "test" {
  ...
  description = "Test validation - missing identify_condition"
}
`, name)
}

func testAccResourceAppRule_validation_all_star_fields(name string) string {
	return fmt.Sprintf(`
resource "huaweicloud_workspace_app_rule" "test" {
  name = "%[1]s"
  rule {
  ...
  description = "Test validation - all star fields"
}
`, name)
}
