...
locals {
  is_flavor_id_filter_useful = [for v in data.huaweicloud_workspace_app_server_quotas.filter_by_flavor_id.quotas : v if v.type == "VOLUMES"]
}

output "is_flavor_id_filter_useful" {
  value = length(local.is_flavor_id_filter_useful) > 0 && local.is_flavor_id_filter_useful[0].remainder > 0 && local.is_flavor_id_filter_useful[0].need > 0
}
