...
func TestAccDataSourceCassandraSlowLogs_basic(t *testing.T) {
	var (
		dataSource = "data.huaweicloud_geminidb_cassandra_slow_logs.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceCassandraSlowLogs_basic(start_time, end_time),
...
func testAccDataSourceCassandraSlowLogs_basic(startTime, endTime string) string {
	return fmt.Sprintf(`
...
output "value_is_empty" {
  value = length(data.huaweicloud_geminidb_cassandra_slow_logs.test.slow_logs) == 0
}
