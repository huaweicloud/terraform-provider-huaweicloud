...
func testAccCSBSBackupV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			return fmt.Errorf("the backup still exists, which ID is %s", rs.Primary.ID)
		}
	}
...
func testAccCSBSBackupV1Exists(n string, backups *backup.Backup) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if !ok {
			return fmt.Errorf("the backup %s not found", n)
		}
		...
		if found.Id != rs.Primary.ID {
			return fmt.Errorf("the backup is not found, which ID is %s", rs.Primary.ID)
		}
...
`, rName, acceptance.HW_IMAGE_ID, acceptance.HW_AVAILABILITY_ZONE, acceptance.HW_FLAVOR_ID, acceptance.HW_NETWORK_ID, rName)
}
