...
func testAccCheckCSBSBackupV1DataSourceID(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return errors.New("backup data source ID not set")
		}
...
`, acceptance.HW_IMAGE_ID, acceptance.HW_AVAILABILITY_ZONE, acceptance.HW_FLAVOR_ID, acceptance.HW_NETWORK_ID)
