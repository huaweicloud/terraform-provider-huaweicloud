...
func getCCEProtectionFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	if clusterRespBody == nil {
		return nil, fmt.Errorf("error retrieving CCE protection configuration: configuration is empty in API response")
	}

	protectStatus := utils.PathSearch("protect_status", clusterRespBody, "").(string)
	if protectStatus == "" {
		return nil, fmt.Errorf("error retrieving CCE protection configuration: protect_status is not found in API response")
	}
