...
func TestAccDataSourceGaussDbQuotas_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_quotas.test"
...
func testDataSourceGaussdbOpengaussQuotas_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_gaussdb_quotas" "test" {}

locals {
  enterprise_project_id = "%[1]s"
}
data "huaweicloud_gaussdb_quotas" "enterprise_project_id_filter" {
  enterprise_project_id = "%[1]s"
}
output "enterprise_project_id_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_quotas.enterprise_project_id_filter.eps_quotas) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_quotas.enterprise_project_id_filter.eps_quotas[*].enterprise_project_id :
