...
func TestAccGaussDbInstancePasswordReset_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testGaussdbInstancePasswordReset_basic(rName),
			},
		},
	})
}

func testGaussdbInstancePasswordReset_base(rName string) string {
	return fmt.Sprintf(`
%[1]s
...
func testGaussdbInstancePasswordReset_basic(rName string) string {
	return fmt.Sprintf(`
%s

resource "huaweicloud_gaussdb_instance_password_reset" "test" {
  instance_id = huaweicloud_gaussdb_instance.test.id
  password    = "test_1234_update"
}
`, testGaussdbInstancePasswordReset_base(rName))
}
