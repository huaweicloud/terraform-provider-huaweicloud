...
resource "huaweicloud_gaussdb_instance" "source" {
  depends_on = [
...
resource "huaweicloud_gaussdb_instance" "target" {
  depends_on = [
    huaweicloud_networking_secgroup_rule.in_v4_tcp_opengauss,
    huaweicloud_networking_secgroup_rule.in_v4_tcp_opengauss_egress
  ]
...
resource "huaweicloud_gaussdb_backup" "backup" {
  instance_id = huaweicloud_gaussdb_instance.source.id
  name        = "%[2]s_backup"
}

resource "huaweicloud_gaussdb_restore" "test" {
  target_instance_id = huaweicloud_gaussdb_instance.target.id
  source_instance_id = huaweicloud_gaussdb_instance.source.id
  type               = "backup"
  backup_id          = huaweicloud_gaussdb_backup.backup.id
}
...
data "huaweicloud_gaussdb_restore_time_ranges" "restore_time" {
  depends_on = [huaweicloud_gaussdb_backup.backup]

  instance_id = huaweicloud_gaussdb_instance.source.id
  date        = split("T", huaweicloud_gaussdb_backup.backup.end_time)[0]
}
...
resource "huaweicloud_gaussdb_restore" "test" {
  ...
  source_instance_id = huaweicloud_gaussdb_instance.source.id
  type               = "timestamp"
  restore_time       = data.huaweicloud_gaussdb_restore_time_ranges.restore_time.restore_time[0].start_time
}
`, testAccOpenGaussRestoreonfig_base(name), name, acceptance.HW_ENTERPRISE_PROJECT_ID_TEST)
}
