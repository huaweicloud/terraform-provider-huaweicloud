...
func TestAccDataSourceGaussDbFlavors_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_flavors.test"
...
locals {
  version = data.huaweicloud_gaussdb_flavors.test.flavors[0].version
}
data "huaweicloud_gaussdb_flavors" "version_filter" {
  version = data.huaweicloud_gaussdb_flavors.test.flavors[0].version
}

output "version_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_flavors.version_filter.flavors) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_flavors.version_filter.flavors[*].version : v == local.version]
  )
}

locals {
  spec_code = data.huaweicloud_gaussdb_flavors.test.flavors[0].spec_code
}
data "huaweicloud_gaussdb_flavors" "spec_code_filter" {
  spec_code = data.huaweicloud_gaussdb_flavors.test.flavors[0].spec_code
}

output "spec_code_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_flavors.spec_code_filter.flavors) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_flavors.spec_code_filter.flavors[*].spec_code : v == local.spec_code]
  )
}
...
data "huaweicloud_gaussdb_flavors" "ha_mode_filter" {
  ha_mode = "centralization_standard"
}

output "ha_mode_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_flavors.ha_mode_filter.flavors) > 0
}
`
}
