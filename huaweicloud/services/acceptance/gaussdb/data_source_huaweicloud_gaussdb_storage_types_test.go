...
func TestAccDataSourceGaussDbStorageTypes_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_storage_types.test"
...
func testDataSourceGaussdbOpengaussStorageTypes_basic() string {
	return `
data "huaweicloud_gaussdb_storage_types" "test" {
  version = "8.201"
  ha_mode = ""
}
...
data "huaweicloud_gaussdb_storage_types" "ha_mode_filter" {
  version = "8.201"
  ha_mode = "centralization_standard"
}

output "ha_mode_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_storage_types.ha_mode_filter.storage_type) > 0
}
`
}
