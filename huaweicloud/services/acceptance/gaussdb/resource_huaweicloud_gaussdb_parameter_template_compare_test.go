...
func TestAccOpenGaussParameterTemplateCompare_basic(t *testing.T) {
	...
	rName := "huaweicloud_gaussdb_parameter_template_compare.test"
...
resource "huaweicloud_gaussdb_parameter_template" "target" {
  name           = "%[1]s_target"
  engine_version = "8.201"
  instance_mode  = "independent"
...
resource "huaweicloud_gaussdb_parameter_template_compare" "test" {
  source_id = huaweicloud_gaussdb_parameter_template.source.id
  target_id = huaweicloud_gaussdb_parameter_template.target.id
}
`, name)
}
