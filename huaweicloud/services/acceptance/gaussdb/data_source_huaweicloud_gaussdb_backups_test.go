...
func TestAccDataSourceGaussDbBackups_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_backups.test"
...
output "instance_id_filter_is_useful" {
  ...
  [for v in data.huaweicloud_gaussdb_backups.instance_id_filter.backups[*].instance_id : v == local.instance_id]
  )
}
...
output "backup_id_filter_is_useful" {
  ...
  [for v in data.huaweicloud_gaussdb_backups.backup_id_filter.backups[*].id : v == local.backup_id]
  )
}
...
output "backup_type_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_backups.backup_type_filter.backups) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_backups.backup_type_filter.backups[*].type : v == local.backup_type]
  )
}
