...
func TestAccDataSourceGaussDbInstanceNodes_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_instance_nodes.test"
...
data "huaweicloud_gaussdb_instance_nodes" "component_type_filter" {
  instance_id    = huaweicloud_gaussdb_instance.test.id
  component_type = "DN"
}
output "component_type_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_instance_nodes.component_type_filter.nodes) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_instance_nodes.component_type_filter.nodes[*] : alltrue(
