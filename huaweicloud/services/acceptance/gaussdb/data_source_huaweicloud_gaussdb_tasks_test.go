...
func TestAccDataSourceGaussDbTasks_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_tasks.test"
...
data "huaweicloud_gaussdb_tasks" "test" {
  depends_on = [huaweicloud_gaussdb_instance.test]
}

locals {
  status = "Completed"
}
data "huaweicloud_gaussdb_tasks" "status_filter" {
  depends_on = [
...
output "status_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_tasks.status_filter.tasks) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_tasks.status_filter.tasks[*].status : v == local.status]
  )
}

locals {
  name = data.huaweicloud_gaussdb_tasks.test.tasks[0].name
}
data "huaweicloud_gaussdb_tasks" "name_filter" {
  depends_on = [
    data.huaweicloud_gaussdb_tasks.test
  ]
...
output "name_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_tasks.name_filter.tasks) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_tasks.name_filter.tasks[*].name : v == local.name]
  )
}

locals {
  start_time = data.huaweicloud_gaussdb_tasks.test.tasks[0].created_at
  end_time   = data.huaweicloud_gaussdb_tasks.test.tasks[0].ended_at
}
data "huaweicloud_gaussdb_tasks" "time_filter" {
  depends_on = [
    data.huaweicloud_gaussdb_tasks.test
  ]
...
output "time_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_tasks.time_filter.tasks) > 0
}
