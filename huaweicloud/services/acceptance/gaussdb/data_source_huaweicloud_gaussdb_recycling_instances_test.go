...
func TestAccDataSourceGaussDbRecyclingInstances_basic(t *testing.T) {
	dataSource := "data.huaweicloud_gaussdb_recycling_instances.test"
...
func testDataSourceGaussdbOpengaussRecyclingInstances_basic(rName string) string {
	return fmt.Sprintf(`
data "huaweicloud_gaussdb_recycling_instances" "test" {}

locals {
  instance_name = "%[1]s"
}
data "huaweicloud_gaussdb_recycling_instances" "instance_name_filter" {
  instance_name = "%[1]s"
}
output "instance_name_filter_is_useful" {
  value = length(data.huaweicloud_gaussdb_recycling_instances.instance_name_filter.instances) > 0 && alltrue(
  [for v in data.huaweicloud_gaussdb_recycling_instances.instance_name_filter.instances[*] : v.name == local.instance_name]
  )
}
`, rName)
}
