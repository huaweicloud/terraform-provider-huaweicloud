...
func TestAccDataSourceDmsKafkaBackgroundTasks_basic(t *testing.T) {
	dataSource := "data.huaweicloud_dms_kafka_background_tasks.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceDmsKafkaBackgroundTasks_basic(rName),
...
func testDataSourceDmsKafkaBackgroundTasks_basic(name string) string {
...
`, testAccKafkaInstance_newFormat(name), beginTime, endTime)
}
