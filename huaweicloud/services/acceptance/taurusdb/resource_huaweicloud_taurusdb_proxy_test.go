...
func TestAccTaurusDBMySQLProxy_basic(t *testing.T) {
	var proxy instances.Proxy
	rName := acceptance.RandomAccResourceName()
	updateName := acceptance.RandomAccResourceName()
	resourceName := "huaweicloud_taurusdb_proxy.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "flavor",
						"data.huaweicloud_taurusdb_proxy_flavors.test", "flavor_groups.0.flavors.0.spec_code"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "instance_id",
						"huaweicloud_taurusdb_instance.test", "id"),
					resource.TestCheckResourceAttrPair(resourceName, "flavor",
						"data.huaweicloud_taurusdb_proxy_flavors.test", "flavor_groups.0.flavors.1.spec_code"),
			...
			},
			{
				...
				ImportStateIdFunc: testTaurusDBProxyResourceImportState(resourceName),
...
resource "huaweicloud_taurusdb_proxy" "test" {
  instance_id              = huaweicloud_taurusdb_instance.test.id
  flavor                   = data.huaweicloud_taurusdb_proxy_flavors.test.flavor_groups[0].flavors[1].spec_code
  node_num                 = 4
  proxy_name               = "%[2]s"
  proxy_mode               = "readwrite"
  route_mode               = 1
...
func testTaurusDBProxyResourceImportState(name string) resource.ImportStateIdFunc {
