...
func getTaurusDBEipAssociateResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error retrieving EIP associated with TaurusDB: %s", err)
	}
...
func TestAccTaurusDBEipAssociate_basic(t *testing.T) {
	var obj interface{}

	name := acceptance.RandomAccResourceName()
	rName := "huaweicloud_taurusdb_eip_associate.test"

	rc := acceptance.InitResourceCheck(
		rName,
		&obj,
		getTaurusDBEipAssociateResourceFunc,
	)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testTaurusDBEipAssociate_basic(name),
...
func testTaurusDBEipAssociate_basic(rName string) string {
	return fmt.Sprintf(`
%[1]s
