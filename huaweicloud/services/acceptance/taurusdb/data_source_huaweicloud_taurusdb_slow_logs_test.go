...
func TestAccDataSourceTaurusDBSlowLogs_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_slow_logs.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBSlowLogs_basic(),
...
func testDataSourceTaurusDBSlowLogs_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_taurusdb_slow_logs" "test" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
data "huaweicloud_taurusdb_slow_logs" "operate_type_filter" {
  instance_id  = "%[1]s"
  node_id      = "%[2]s"
  start_time   = "%[3]s"
  end_time     = "%[4]s"
...
output "operate_type_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_slow_logs.operate_type_filter.slow_log_list) > 0 && alltrue(
  [for v in data.huaweicloud_taurusdb_slow_logs.operate_type_filter.slow_log_list[*].type : v == local.operate_type]
  )
}
...
data "huaweicloud_taurusdb_slow_logs" "database_filter" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
output "database_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_slow_logs.database_filter.slow_log_list) > 0 && alltrue(
