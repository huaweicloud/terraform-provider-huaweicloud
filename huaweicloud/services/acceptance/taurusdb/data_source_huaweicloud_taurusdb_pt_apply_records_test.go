...
func TestAccDataSourceTaurusDBPtApplyRecords_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_pt_apply_records.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBPtApplyRecords_basic(rName),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(dataSource, "histories.0.target_name",
						"huaweicloud_taurusdb_instance.test", "name"),
...
func testDataSourceTaurusDBPtApplyRecords_base(name string) string {
	return fmt.Sprintf(`
%[1]s
...
func testDataSourceTaurusDBPtApplyRecords_basic(name string) string {
	return fmt.Sprintf(`
%s

data "huaweicloud_taurusdb_pt_apply_records" "test" {
  depends_on = [huaweicloud_taurusdb_parameter_template_apply.test]

  config_id = huaweicloud_taurusdb_parameter_template.test.id
}
`, testDataSourceTaurusDBPtApplyRecords_base(name))
}
