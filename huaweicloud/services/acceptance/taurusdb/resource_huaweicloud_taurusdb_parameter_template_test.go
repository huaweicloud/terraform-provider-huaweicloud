...
func getTaurusDBTemplateResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error retrieving TaurusDB Parameter Template: %s", err)
	}
	return utils.FlattenResponse(getParameterTemplateResp)
}

func TestAccTaurusDBTemplate_basic(t *testing.T) {
	var obj interface{}

	name := acceptance.RandomAccResourceName()
	updateName := acceptance.RandomAccResourceName()
	rName := "huaweicloud_taurusdb_parameter_template.test"

	rc := acceptance.InitResourceCheck(
		rName,
		&obj,
		getTaurusDBTemplateResourceFunc,
	)
...
func TestAccTaurusDBTemplate_with_configuration(t *testing.T) {
	var obj interface{}
...
func TestAccTaurusDBTemplate_with_instance(t *testing.T) {
	var obj interface{}
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			acceptance.TestAccPreCheckTaurusDBInstanceConfigurationId(t)
		},
		ProviderFactories: acceptance.TestAccProviderFactories,
		CheckDestroy:      rc.CheckResourceDestroy(),
		Steps: []resource.TestStep{
...
func testParameterTemplate_with_configuration(name string) string {
	return fmt.Sprintf(`
resource "huaweicloud_taurusdb_parameter_template" "source_template" {
  name              = "%[1]s_source"
  datastore_engine  = "gaussdb-mysql"
  datastore_version = "8.0"
...
resource "huaweicloud_taurusdb_parameter_template" "test" {
  name                    = "%[1]s"
  description             = "test gaussdb mysql parameter template"
  source_configuration_id = huaweicloud_taurusdb_parameter_template.source_template.id
...
`, name, acceptance.HW_TAURUSDB_INSTANCE_ID, acceptance.HW_TAURUSDB_INSTANCE_CONFIGURATION_ID)
}
