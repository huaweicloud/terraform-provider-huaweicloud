...
func TestAccTaurusDBTemplateCompare_basic(t *testing.T) {
	name := acceptance.RandomAccResourceName()
	rName := "huaweicloud_taurusdb_parameter_template_compare.test"
...
func testParameterTemplateCompare_basic(name string) string {
	return fmt.Sprintf(`
resource "huaweicloud_taurusdb_parameter_template" "source" {
  name              = "%[1]s_source"
...
resource "huaweicloud_taurusdb_parameter_template" "target" {
  name              = "%[1]s_target"
  description       = "test gaussdb mysql parameter template"
  datastore_engine  = "gaussdb-mysql"
  datastore_version = "8.0"
...
resource "huaweicloud_taurusdb_parameter_template_compare" "test" {
  source_configuration_id = huaweicloud_taurusdb_parameter_template.source.id
  target_configuration_id = huaweicloud_taurusdb_parameter_template.target.id
}
`, name)
}
