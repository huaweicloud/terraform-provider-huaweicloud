...
func TestAccDataSourceTaurusDBAuditLogDownloadLinks_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_audit_log_download_links.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBAuditLogDownloadLinks_basic(),
...
func testDataSourceTaurusDBAuditLogDownloadLinks_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_taurusdb_audit_log_download_links" "test" {
  instance_id = "%[1]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
}
...
data "huaweicloud_taurusdb_audit_log_download_links" "node_id_filter" {
  instance_id  = "%[1]s"
  node_id      = "%[2]s"
  start_time   = "%[3]s"
  end_time     = "%[4]s"
