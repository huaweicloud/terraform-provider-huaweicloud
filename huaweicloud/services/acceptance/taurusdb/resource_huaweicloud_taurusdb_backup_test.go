...
func getTaurusDBBackupResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error retrieving TaurusDB backup: %s", err)
	}
...
func TestAccTaurusDBBackup_basic(t *testing.T) {
	var obj interface{}

	name := acceptance.RandomAccResourceName()
	rName := "huaweicloud_taurusdb_backup.test"

	rc := acceptance.InitResourceCheck(
		rName,
		&obj,
		getTaurusDBBackupResourceFunc,
	)
...
resource "huaweicloud_taurusdb_backup" "test" {
