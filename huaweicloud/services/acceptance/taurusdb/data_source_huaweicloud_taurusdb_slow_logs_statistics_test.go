...
func TestAccDataSourceTaurusDBSlowLogsStatistics_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_slow_logs_statistics.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBSlowLogsStatistics_basic(),
...
func testDataSourceTaurusDBSlowLogsStatistics_basic() string {
...
locals {
  type     = data.huaweicloud_taurusdb_slow_logs_statistics.test.slow_log_list[0].type
  database = data.huaweicloud_taurusdb_slow_logs_statistics.test.slow_log_list[0].database
}

data "huaweicloud_taurusdb_slow_logs_statistics" "type_filter" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
output "type_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_slow_logs_statistics.type_filter.slow_log_list) > 0 && alltrue(
  [for v in data.huaweicloud_taurusdb_slow_logs_statistics.type_filter.slow_log_list[*].type : v == local.type]
  )
}

data "huaweicloud_taurusdb_slow_logs_statistics" "database_filter" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
output "database_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_slow_logs_statistics.database_filter.slow_log_list) > 0 && alltrue(
  [for v in data.huaweicloud_taurusdb_slow_logs_statistics.database_filter.slow_log_list[*].database : v == local.database]
  )
}
`, acceptance.HW_TAURUSDB_INSTANCE_ID, acceptance.HW_TAURUSDB_NODE_ID, startTime, endTime)
}
