...
func getTaurusDBRecyclingPolicyResourceFunc(cfg *config.Config, _ *terraform.ResourceState) (interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error retrieving TaurusDB recycling policy: %s", err)
	}
	...
	if retentionPeriod == nil {
		return nil, errors.New("error retrieving TaurusDB recycling policy, retention_period_in_days is not found")
	}
	return getRespBody, nil
}

func TestAccTaurusDBRecyclingPolicy_basic(t *testing.T) {
	var obj interface{}

	rName := "huaweicloud_taurusdb_recycling_policy.test"

	rc := acceptance.InitResourceCheck(
		rName,
		&obj,
		getTaurusDBRecyclingPolicyResourceFunc,
	)
...
func testRecyclingPolicy_basic(retentionPeriodInDays string) string {
	return fmt.Sprintf(`
resource "huaweicloud_taurusdb_recycling_policy" "test" {
