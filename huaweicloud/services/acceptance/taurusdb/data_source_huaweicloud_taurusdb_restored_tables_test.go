...
func TestAccDataSourceTaurusDBRestoredTables_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_restored_tables.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBRestoredTables_basic(),
...
func testDataSourceTaurusDBRestoredTables_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_taurusdb_restore_time_ranges" "test" {
  instance_id = "%[1]s"
}

data "huaweicloud_taurusdb_restored_tables" "test" {
  instance_id     = "%[1]s"
  restore_time    = data.huaweicloud_taurusdb_restore_time_ranges.test.restore_times[0].end_time
...
data "huaweicloud_taurusdb_restored_tables" "database_name_filter" {
  instance_id     = "%[1]s"
  restore_time    = data.huaweicloud_taurusdb_restore_time_ranges.test.restore_times[0].end_time
  last_table_info = true
  database_name   = "%[2]s"
...
output "database_name_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_restored_tables.database_name_filter.databases) > 0
}

locals {
  table_name = "%[3]s"
}
data "huaweicloud_taurusdb_restored_tables" "table_name_filter" {
  instance_id     = "%[1]s"
  restore_time    = data.huaweicloud_taurusdb_restore_time_ranges.test.restore_times[0].end_time
  last_table_info = true
  table_name      = "%[3]s"
...
output "table_name_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_restored_tables.table_name_filter.databases) > 0 && alltrue(
