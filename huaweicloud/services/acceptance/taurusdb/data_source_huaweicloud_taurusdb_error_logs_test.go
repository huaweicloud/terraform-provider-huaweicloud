...
func TestAccDataSourceTaurusDBErrorLogs_basic(t *testing.T) {
	dataSource := "data.huaweicloud_taurusdb_error_logs.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceTaurusDBErrorLogs_basic(),
...
func testDataSourceTaurusDBErrorLogs_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_taurusdb_error_logs" "test" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
data "huaweicloud_taurusdb_error_logs" "level_filter" {
  instance_id = "%[1]s"
  node_id     = "%[2]s"
  start_time  = "%[3]s"
  end_time    = "%[4]s"
...
output "level_filter_is_useful" {
  value = length(data.huaweicloud_taurusdb_error_logs.level_filter.error_log_list) > 0 && alltrue(
  [for v in data.huaweicloud_taurusdb_error_logs.level_filter.error_log_list[*].level : v == local.level]
  )
}
