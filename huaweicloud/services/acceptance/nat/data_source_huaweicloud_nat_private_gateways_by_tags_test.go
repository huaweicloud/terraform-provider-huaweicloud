...
func TestAccDataSourceNatPrivateGatewaysByTags_basic(t *testing.T) {
	var (
		filterByTagsAll    = "data.huaweicloud_nat_private_gateways_by_tags.filter_by_tags"
		dcTags             = acceptance.InitDataSourceCheck(filterByTagsAll)
		filterByMatchesAll = "data.huaweicloud_nat_private_gateways_by_tags.filter_by_matches"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceNatPrivateGatewaysByTags_basic(testName),
...
func testDataSourceNatPrivateGatewaysByTags_base(name string) string {
	return fmt.Sprintf(`
...
func testDataSourceNatPrivateGatewaysByTags_basic(name string) string {
	return fmt.Sprintf(`
%[1]s

data "huaweicloud_nat_private_gateways_by_tags" "filter_by_tags" {
...
output "tags_filter_is_useful" {
  value = length(data.huaweicloud_nat_private_gateways_by_tags.filter_by_tags.resources) > 0 && alltrue(
    [for v in data.huaweicloud_nat_private_gateways_by_tags.filter_by_tags.resources[*].tags : anytrue(
    [for vv in v[*].key : vv == local.tag_key]) && anytrue([for vv in v[*].value : vv == local.tag_value])]
  )
}

data "huaweicloud_nat_private_gateways_by_tags" "filter_by_matches" {
  depends_on = [huaweicloud_nat_private_gateway.test]
  action     = "filter"
...
output "matches_filter_is_useful" {
  value = length(data.huaweicloud_nat_private_gateways_by_tags.filter_by_matches.resources) > 0
}

`, testDataSourceNatPrivateGatewaysByTags_base(name), name)
}
