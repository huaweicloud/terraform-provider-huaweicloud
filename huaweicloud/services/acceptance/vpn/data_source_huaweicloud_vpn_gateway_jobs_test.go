...
func TestAccDataSourceVpnGatewayJobs_basic(t *testing.T) {
	dataSource := "data.huaweicloud_vpn_gateway_jobs.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceVpnGatewayJobs_basic,
...
const testDataSourceVpnGatewayJobs_basic = `
data "huaweicloud_vpn_gateway_jobs" "test" {}

locals {
  resource_id = data.huaweicloud_vpn_gateway_jobs.test.jobs[0].resource_id
}

// filter by resource_id
data "huaweicloud_vpn_gateway_jobs" "filter_by_resource_id" {
  resource_id = local.resource_id
}

locals {
  filter_result_by_resource_id = [for v in data.huaweicloud_vpn_gateway_jobs.filter_by_resource_id.jobs[*].resource_id :
    v == local.resource_id]
}
