...
resource "huaweicloud_vpn_user_group" "test" {
  ...
  users {
    id = huaweicloud_vpn_user.user1.id
  }

  users {
    id = huaweicloud_vpn_user.user2.id
  }
}
...
resource "huaweicloud_vpn_user_group" "test" {
  ...
  users {
    id = huaweicloud_vpn_user.user3.id
  }
}
`, testAccUserGroup_base(name), acceptance.HW_VPN_P2C_SERVER, name)
}
...
func testAccUserGroup_base(name string) string {
	return fmt.Sprintf(`
resource "huaweicloud_vpn_user" "user1" {
  vpn_server_id = "%[1]s"
  name          = "%[2]s1"
  password      = "Typhoeus121"
  description   = "test"
}
	  
resource "huaweicloud_vpn_user" "user2" {
  vpn_server_id = "%[1]s"
  name          = "%[2]s2"
  password      = "Typhoeus122"
  description   = "test"
}
	  
resource "huaweicloud_vpn_user" "user3" {
  vpn_server_id = "%[1]s"
  name          = "%[2]s3"
