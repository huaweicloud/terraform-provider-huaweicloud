...
func TestAccDataRocketmqBackgroundTasks_basic(t *testing.T) {
	var (
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccDataRocketmqBackgroundTasks_notFound(),
				ExpectError: regexp.MustCompile(`This DMS instance does not exist`),
			},
			{
				Config: testAccDataRocketmqBackgroundTasks_basic(),
...
func testAccDataRocketmqBackgroundTasks_notFound() string {
...
func testAccDataRocketmqBackgroundTasks_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_dms_rocketmq_background_tasks" "test" {
  instance_id = "%[1]s"
}
