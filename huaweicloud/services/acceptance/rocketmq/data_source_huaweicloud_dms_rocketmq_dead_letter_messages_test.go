...
func TestAccDataSourceRocketMQDeadLetterMessages_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			acceptance.TestAccPreCheckDMSRocketMQDeadLetterMessageIDs(t)
		},
		ProviderFactories: acceptance.TestAccProviderFactories,
		Steps: []resource.TestStep{
			{
				Config:      testAccRocketMQDeadLetterMessages_instanceNotFound(),
				ExpectError: regexp.MustCompile(`This DMS instance does not exist`),
			},
			{
				Config:      testAccRocketMQDeadLetterMessages_topicNotFound(),
				ExpectError: regexp.MustCompile(`Query topic failed. No topic route info in name server for topic`),
			},
			{
				Config: testDataSourceRocketMQDeadLetterMessages_basic(),
...
func testAccRocketMQDeadLetterMessages_instanceNotFound() string {
...
func testAccRocketMQDeadLetterMessages_topicNotFound() string {
	return fmt.Sprintf(`
data "huaweicloud_dms_rocketmq_dead_letter_messages" "test" {
  instance_id = "%[1]s"
  topic       = "not_found"
...
func testDataSourceRocketMQDeadLetterMessages_basic() string {
	return fmt.Sprintf(`
data "huaweicloud_dms_rocketmq_dead_letter_messages" "test" {
  instance_id = "%[1]s"
  topic       = "%%DLQ%%%[2]s"
