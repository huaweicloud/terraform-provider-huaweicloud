...
// getAuthorizationTokenHere is a method to request the CSE API and get the authorization token.
// The format of is "Bearer {token}".
func getAuthorizationTokenHere(c *golangsdk.ServiceClient, username, password string) (string, error) {
...
func GetAuthorizationToken(connAddr, username, password string) (string, error) {
	...
	return getAuthorizationTokenHere(client, username, password)
}
