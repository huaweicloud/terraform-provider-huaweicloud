...
func resourceCsClusterV1Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error building the request body of API(update): %s", err)
	}
	if e, _ := isEmptyValue(reflect.ValueOf(params)); !e {
		_, err = sendCsClusterV1UpdateRequest(d, params, client)
		if err != nil {
			return fmt.Errorf("error updating CS cluster, which ID is %s: %s", d.Id(), err)
		}
	}
...
func sendCsClusterV1UpdateRequest(d *schema.ResourceData, params interface{},
	...
	if r.Err != nil {
		return nil, fmt.Errorf("error running API(update): %s", r.Err)
	}
	return r.Body, nil
}
...
func sendCsClusterV1ReadRequest(d *schema.ResourceData, client *golangsdk.ServiceClient) (interface{}, error) {
	...
	_, r.Err = client.Get(url, &r.Body, &golangsdk.RequestOpts{
		MoreHeaders: map[string]string{"Content-Type": "application/json"}})
	if r.Err != nil {
		return nil, fmt.Errorf("error running API(read) for resource(CS cluster): %s", r.Err)
	}
...
func flattenCsClusterV1Options(response map[string]interface{}) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:created_at: %s", err)
	}
	opts["created_at"] = v

	v, err = navigateValue(response, []string{"read", "desc"}, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:description: %s", err)
	}
	opts["description"] = v

	v, err = flattenCsClusterV1ManagerNodeSpuNum(response, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:manager_node_spu_num: %s", err)
	}
	opts["manager_node_spu_num"] = v

	v, err = flattenCsClusterV1MaxSpuNum(response, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:max_spu_num: %s", err)
	}
	opts["max_spu_num"] = v

	v, err = navigateValue(response, []string{"read", "name"}, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:name: %s", err)
	}
	opts["name"] = v

	v, err = navigateValue(response, []string{"read", "subnet_cidr"}, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:subnet_cidr: %s", err)
	}
	opts["subnet_cidr"] = v

	v, err = navigateValue(response, []string{"read", "subnet_gateway"}, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:subnet_gateway: %s", err)
	}
	opts["subnet_gateway"] = v

	v, err = flattenCsClusterV1UsedSpuNum(response, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:used_spu_num: %s", err)
	}
	opts["used_spu_num"] = v

	v, err = navigateValue(response, []string{"read", "vpc_cidr"}, nil)
	if err != nil {
		return nil, fmt.Errorf("error flattening CS cluster:vpc_cidr: %s", err)
	}
	opts["vpc_cidr"] = v
