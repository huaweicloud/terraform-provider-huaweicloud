...
func resourceCssClusterV1Read(d *schema.ResourceData, meta interface{}) error {
	...
	if resourceTags, err := tags.Get(client, "css-cluster", d.Id()).Extract(); err == nil {
		...
		if err := d.Set("tags", tagmap); err != nil {
			return fmt.Errorf("Error saving tag to state for HuaweiCloud CSS cluster (%s): %s", d.Id(), err)
		}
	} else {
		log.Printf("[WARN] Error fetching tags of HuaweiCloud CSS cluster (%s): %s", d.Id(), err)
	}
