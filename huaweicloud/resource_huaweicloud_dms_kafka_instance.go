...
func ResourceDmsKafkaInstance() *schema.Resource {
	return &schema.Resource{
		...
		Update: resourceDmsKafkaInstanceUpdate,
		Delete: resourceDmsKafkaInstanceDelete,
...
func resourceDmsKafkaInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceDmsKafkaInstanceRead(d, meta)
}

func resourceDmsKafkaInstanceRead(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*config.Config)
	...
	partitionNum, _ := convertToInt(v.PartitionNum)
...
func resourceDmsKafkaInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*config.Config)
	dmsV2Client, err := config.DmsV2Client(GetRegion(d, config))
	if err != nil {
		return fmtp.Errorf("error creating HuaweiCloud dms instance client: %s", err)
...
func resourceDmsKafkaInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*config.Config)
	dmsV2Client, err := config.DmsV2Client(GetRegion(d, config))
	if err != nil {
		return fmtp.Errorf("error creating HuaweiCloud dms instance client: %s", err)
