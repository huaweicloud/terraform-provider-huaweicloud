...
func resourceInstanceRootVolumeV1(d *schema.ResourceData, config *Config) cloudservers.RootVolume {
	disk_type := d.Get("system_disk_type").(string)
	if disk_type == "" {
		disk_type = config.DefaultArgumentValues["compute_instance"]["system_disk_type"].(string)
	}
